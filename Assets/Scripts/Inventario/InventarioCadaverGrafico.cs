...
/// <summary>
/// Muestra el contenido del cadáver abierto en un panel de slots y configura el drag & drop.
/// Asignar al GameObject del panel "Inventario Cadáver" y enlazar el Inventario del personaje.
/// </summary>
public class InventarioCadaverGrafico : MonoBehaviour
{
    public List<Image> celdas;
    [Tooltip("Inventario del personaje (para acceder a CadaverAbierto y mover ítems).")]
    ...
    void ConfigurarDragEnSlots()
    {
        ...
        for (int i = 0; i < celdas.Count; i++)
        {
            ...
            var slot = celdas[i].GetComponent<SlotCadaverDrag>();
            if (slot == null)
                slot = celdas[i].gameObject.AddComponent<SlotCadaverDrag>();
            slot.slotIndex = i;
            slot.inventarioCadaverGrafico = this;
        }
    }

    public void Refrescar()
    {
        if (inventario == null || inventario.CadaverAbierto == null || celdas == null) return;
        CadaverInteractuable cadaver = inventario.CadaverAbierto;
        for (int i = 0; i < celdas.Count; i++)
        {
            if (celdas[i] == null) continue;
            GameObject o = cadaver.GetContenido(i);
            if (o != null)
            {
                ...
                if (ob != null)
                {
                    ...
                    if (ob.categoria == CategoriaObjeto.Acumulable)
                        cantidad = cadaver.GetCantidad(i);
